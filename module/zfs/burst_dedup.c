...
htddt_alloc(const htddt_key_t *htddk)
{
	...
	memset(htdde, 0, sizeof (htddt_entry_t));
...
htddt_bstp_fill(htddt_entry_t *htdde, bstt_phys_t *bstp)
{
	...
	memset(bstp, 0, sizeof (bstt_phys_t));
...
bstt_alloc(const bstt_key_t *bstk)
{
	...
	memset(bste, 0, sizeof (bstt_entry_t));
	bste->bste_key = *bstk;
...
#if defined(_KERNEL)
static int
param_set_burst_htdde_right_shift(const char *buf, zfs_kernel_param_t *kp)
{
	unsigned long val;
	int error;
...
static int
param_set_burst_max_htdde_refcnt(const char *buf, zfs_kernel_param_t *kp)
{
	unsigned long val;
	int error;
...
module_param_call(zfs_burst_htdde_right_shift, param_set_burst_htdde_right_shift,
...
module_param_call(zfs_burst_max_htdde_refcnt, param_set_burst_max_htdde_refcnt,
